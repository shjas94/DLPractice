...
class SEResidualBlock(tf.keras.layers.Layer):
    def __init__(self, channel_in, channel_out, kernel_size, ratio, regularizer=None, **kwargs):
        super().__init__(**kwargs)
        ...
        self.conv2 = tf.keras.layers.Conv2D(
            channel_out, kernel_size=kernel_size, kernel_regularizer=regularizer, kernel_initializer='he_normal', padding='same')
        self.se = SEBlock(channel_out, 16)
...
class SEDeepResidualBlock(tf.keras.layers.Layer):
    def __init__(self, channel_in, channel_out, kernel_size, regularizer=None, reduction_ratio=0.3, **kwargs):
        super().__init__(**kwargs)
        self.bn1 = tf.keras.layers.BatchNormalization()
        self.ac1 = tf.nn.swish()
        self.conv1 = tf.keras.layers.Conv2D(tf.math.floor(channel_out*reduction_ratio), kernel_size=(
            1, 1), kernel_regularizer=regularizer, kernel_initializer='he_normal', padding='same')
        self.bn2 = tf.keras.layers.BatchNormalization()
        self.ac2 = tf.nn.swish()
        self.conv2 = tf.keras.layers.Conv2D(tf.math.floor(channel_out*reduction_ratio), kernel_size=kernel_size,
